...
#            model.add(Dense(self.classes,name='Softmax', activation='softmax'))
#            print('============================',inputs.shape)
#            model.fit(inputs)
#            print(model.summary())
#            print("***********************")
#            final_out = model(inputs)
            clasify = Dense(self.classes,name='Softmax', activation='softmax')
...
#        default_batch_size = 10
    ...
    def measures(self):
        yhat_classes = self.pred_binarisation()
        testy = self.y_test_ori
    ...
    def error_criterion(self):
        ...
        #print("mean_absolute_error(self.y_test, y_pred):", mean_absolute_error(self.y_test, y_pred))
        #print("self._err_delta:",self._err_delta)
        #print("error criterion:",mean_absolute_error(self.y_test, y_pred) <= self._err_delta)
    ...
    def if_part_criterion(self):
        ...
        #self._ifpart_thresh = 0.99 #diabets
        #print(" hada np.max(fuzz_out, axis=-1):",np.max(fuzz_out, axis=-1))
        
        maxes = np.max(fuzz_out, axis=-1) >= self._ifpart_thresh
#        print("***fuzz_out***:",fuzz_out)
        # return True if at least half of samples agree
#        self._ifpart_samples=0.9
        #self._ifpart_samples=0.95 #diabets
        #print("maxes.sum() / len(maxes)",maxes.sum() / len(maxes))
        #print("self._ifpart_samples",self._ifpart_samples)
        #print("if part creterion: ",(maxes.sum() / len(maxes)) >= self._ifpart_samples)
